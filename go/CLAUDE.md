...
Run `make check`. It does all of these:

1. `gofmt -l .` — no output (all formatted)
2. `go vet ./...` — zero warnings
3. `go test ./...` — all pass on macOS (native)
4. `docker run ... go test ./...` — all pass on Linux
5. Cross-compile for darwin/amd64, darwin/arm64, linux/amd64, linux/arm64
